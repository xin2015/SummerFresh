...
namespace SummerFresh.Data.Mapping.Provider
{
    public abstract class GenericMappingProvider : IMappingProvider
    {
        ...
        #region 读取数据库信息通用实现
        public virtual Tables ReadTables(Dao dao)
        {
            return ReadTablesFromDatabase(dao);
        }

        protected Tables ReadTablesFromDatabase(Dao dao)
        {
            ...
            if (tables.Count > 0)
            {
                ...
                //Logger.Debug("read {0} columns", columns.Count());

                IEnumerable<Key> keys = ReadKeys(dao);
                //Logger.Debug("read {0} keys", keys.Count());
                    ...
                    keys.ForEach(key =>
                    {
                        if (key.Table.Equals(table.Name, StringComparison.OrdinalIgnoreCase))
                        {
                                ...
                                table.Schema.Equals(key.Schema, StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                if (null == keyColumn)
                                {
                                    throw new DaoException(string.Format("key column '{0}' not exist in table '{1}'", key.Column, table.Name));
                                }
        ...
        {
            SqlCommandBuilder sql = new SqlCommandBuilder(mapping.DaoProvider);
        ...
        public ISqlCommand CreateSelectAllCommand(TableMapping mapping, object parameter = null)
        {
            SqlCommandBuilder sql = new SqlCommandBuilder(mapping.DaoProvider);
        ...
        {
            IDaoProvider daoProvider = mapping.DaoProvider;
            ...
            mapping.InsertColumns.ForEach(col =>
            {
                if (!col.IsAutoIncrement)
                {
                    ...
                    if (parameters.TryResolve(col.Name, out value))
                    {
                        ...
                        values.Append(string.Format(daoProvider.NamedParameterFormat, paramName));
                ...
                if (!col.IsKey && (null == fields || fields.Length == 0 || (inclusive && contains(col, fields)) || !inclusive && !contains(col, fields)))
                {
                    ...
                    if (parameters.TryResolve(col.Name, out value))
                    {
                        ...
                        sql.AppendCommandText(EscapeIdentifier(col.Name))
                           .AppendCommandText("=")
                           .AppendCommandText(string.Format(daoProvider.NamedParameterFormat, paramName));
        ...
        private bool contains(Column col, String[] fields)
        {
            ...
            if (null != fields)
            {
                ...
                {
                    if (col.Name.Equals(field, StringComparison.OrdinalIgnoreCase) || col.Property.Name.Equals(field, StringComparison.OrdinalIgnoreCase))
                    {
                        contains = true;
                        break;
                    }
        ...
        protected virtual object GetKeyParams(TableMapping mapping, object id)
        {
            ...
            else if (tableKeyCount > 1)
            {
                if (id.GetType().Equals(typeof(string)) || id.GetType().IsPrimitive || id.GetType().IsValueType)
                {
                    throw new DaoException(string.Format("table {0} has more than one field as key, so you must pass more then one param by Dictonary or Entity Class", mapping.Table.Name));
                }
                else
                {
                    return id;
            ...
            if (tableKeyCount == 0)
            {
                throw new DaoException(string.Format("the key of table {0} is absent, so you can't call dao's select or delete method", mapping.Table.Name));
            }
            return id;
        }
        ...
        protected virtual void AppendWhere(SqlCommandBuilder sql, TableMapping mapping, object parameter)
        {
            ...
            foreach (var key in param.Keys)
            {
                ...
                {
                    operation = "{0} = {1}".FormatTo(EscapeIdentifier(key), string.Format(daoProvider.NamedParameterFormat, paramName));
                    sql.AddCommandParameter(paramName, value);
                }
        ...
        protected virtual void AppendKeyWheres(SqlCommandBuilder sql, TableMapping mapping, object idparams)
        {
            ...
            column.ForEach(key =>
            {
                ...
                {
                    throw new Exception(string.Format("parameter value of primary key '{0}' not found", key.Name));
                }
            });
        }
