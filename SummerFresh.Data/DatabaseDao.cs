...
namespace SummerFresh.Data
{
    ...
    public class DatabaseDao : GenericDao
    {
        ...
        public override int BatchInsert<T>(System.Collections.Generic.IList<T> data,string tableName=null)
        {
            if (!data.IsNullOrEmpty())
            {
                Type type = typeof(T);
                ...
                {
                    tableName = type.GetCustomAttribute<TableAttribute>(true).Name;
                }
                DataTable table = new DataTable() { TableName=tableName};
